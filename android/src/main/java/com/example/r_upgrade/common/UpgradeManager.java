...
import com.example.r_upgrade.BuildConfig;
...
//                    Log.d(TAG, "queryTask: 下载失败");
   ...
   public boolean  install_apk(File file) {
       Log.d(TAG,"***************Yesssss****************");
        try {
            if (file.exists()) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                        Uri fileUri24 = getFileUri(upgradeManager,file);

                        Intent intent = new Intent(Intent.ACTION_VIEW);
                        intent.setDataAndType(fileUri24,
